...
_COMPILED = tuple((re.compile(rf"(?<!\w){re.escape(phrase)}(?!\w)", re.IGNORECASE), priority, action, hint) for phrase, priority, action, hint in _FLAG_REGISTRY)
...
def detect_red_flags(text: str) -> list[dict]:
    ...
    for pattern, priority, action, icd10_hint in _COMPILED:
        ...
        if match:
            ...
            hits[phrase] = {"phrase": phrase, "priority": priority, "action": action, "icd10_hint": icd10_hint}
