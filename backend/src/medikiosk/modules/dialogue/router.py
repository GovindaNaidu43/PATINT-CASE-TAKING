"""FastAPI router for patient dialogue turns and triage evaluation."""
...
from medikiosk.domain.models import Consultation
from medikiosk.domain.schemas import DialogueTurn
from medikiosk.modules.dialogue.state_machine import InterviewState
from medikiosk.modules.dialogue.repertory import match_rubrics
...
async def add_turn(
    ...
    consultation.status = (
        "priority_review"
        if result["red_flags"]
        else ("awaiting_review" if result["phase"] == "complete" else "active")
    )
    db.add(consultation)
    db.commit()
    await hub.publish(
