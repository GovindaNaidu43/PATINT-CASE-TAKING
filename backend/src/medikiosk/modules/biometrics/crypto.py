...
def _projection(descriptor: list[float], salt: bytes) -> list[int]:
    if len(descriptor) != DESCRIPTOR_SIZE or not all(isinstance(v, (int, float)) for v in descriptor):
        raise ValueError("Face descriptor must contain exactly 128 numeric values")
    magnitude = sum(float(v) ** 2 for v in descriptor) ** 0.5
    if magnitude == 0:
        raise ValueError("Face descriptor cannot be zero")
    normalized = [float(v) / magnitude for v in descriptor]
    bits = []
    for row in range(PROJECTION_SIZE):
        seed = hmac.new(salt, f"projection:{row}".encode(), hashlib.sha256).digest()
        total = sum(
            v * (1.0 if seed[i % len(seed)] & (1 << (i % 8)) else -1.0)
...
def build_template(descriptors: list[list[float]]) -> tuple[str, str, str]:
    ...
    centroid_raw = [
        sum(d[i] for d in descriptors) / len(descriptors) for i in range(DESCRIPTOR_SIZE)
    ]
    centroid = _projection(centroid_raw, salt)
...
def matches(descriptor: list[float], encrypted_template: str, nonce: str, salt: str) -> bool:
    ...
    stored = json.loads(
        AESGCM(_master_key()).decrypt(
            base64.urlsafe_b64decode(nonce),
            base64.urlsafe_b64decode(encrypted_template),
            salt_bytes,
        )
    )
