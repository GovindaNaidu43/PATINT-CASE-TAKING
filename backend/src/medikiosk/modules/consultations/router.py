"""FastAPI router for consultations and prescriptions."""
import uuid
...
from medikiosk.core.security import kiosk_identity, require_physician, require_staff
from medikiosk.domain.models import AuditEvent, Consultation, ConsentRecord, Patient, Prescription
from medikiosk.domain.schemas import ConsultationCreate, PrescriptionCreate
...
async def create_consultation(
    ...
    consultation = Consultation(id=str(uuid.uuid4()), patient_id=payload.patient_id, turns=[])
    db.add(consultation)
    db.commit()
    await hub.publish(
...
def get_consultation(
    ...
    consent = (
        db.execute(
            ...
            .where(ConsentRecord.consultation_id == consultation.id)
    ...
    return {
        ...
        "turns": consultation.turns or [],
        "red_flags": consultation.red_flags or [],
        "consent": {
        ...
        "prescriptions": [prescription_response(item) for item in prescriptions],
    }
...
def create_prescription(
    ...
    consultation = db.get(Consultation, consultation_id)
    if not consultation or payload.consultation_id != consultation_id:
        raise HTTPException(404, "Consultation not found")
...
def sign_prescription(
    ...
    if not prescription or prescription.consultation_id != consultation_id:
