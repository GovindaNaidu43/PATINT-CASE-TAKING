...
from models import AuditEvent, ConsentRecord, Consultation, FollowUp
...
def schedule(data: Schedule, staff: dict = Depends(require_staff), db: Session = Depends(get_db)):
	...
	consent = db.execute(select(ConsentRecord).where(ConsentRecord.consultation_id == consultation.id, ConsentRecord.withdrawn_at.is_(None))).scalars().first()
	if not consent or "followup" not in consent.purposes: raise HTTPException(409, "Active follow-up consent is required")
