...
from models import Consultation, SupportingSignal
...
@router.post("/jihva")
async def jihva(image: UploadFile = File(...), consultation_id: str = Form(...), _: str = Depends(kiosk_identity), db: Session = Depends(get_db)):
    consultation = db.get(Consultation, consultation_id)
    if not consultation: raise HTTPException(404, "Consultation not found")
    try:
        result = analyze_jihva(await image.read())
        signal = SupportingSignal(id=str(uuid.uuid4()), consultation_id=consultation.id, kind="jihva", payload=result)
