...
export interface OrnateMandalaProps {
  color?: string;
  size?: number | string;
  className?: string;
...
/**
 * Classical Ornate Mandala SVG Component (Lotus 16-petal, 8-petal mid layer,
 * diamonds, and geometric star). Shared across Kiosk & Doctor Dashboard.
 */
export const OrnateMandala: React.FC<OrnateMandalaProps> = ({
  color = '#B8863C',
  size = '100%',
  className = '',
  style,
  ...
  const petals16 = Array.from({ length: 16 }, (_, i) => {
    ...
    return `M100,100 C${cx1},${cy1} ${cx2},${cy2} ${x1},${y1} Z`;
  });
  ...
  const innerPetals12 = Array.from({ length: 12 }, (_, i) => {
  ...
  return (
    ...
    >
      ...
      <circle cx="100" cy="100" r="3.5" fill="currentColor" opacity="0.75" />

      {/* 16-petal outer lotus */}
      {petals16.map((d, i) => (
        <path key={`p16-${i}`} d={d} stroke="currentColor" strokeWidth="0.9" fill="none" />
      ))}

      {/* 8-petal mid lotus */}
      {petals8.map((d, i) => (
        <path key={`p8-${i}`} d={d} stroke="currentColor" strokeWidth="1.1" fill="none" />
      ))}

      {/* Star geometric intersection lines */}
      {Array.from({ length: 8 }).map((_, i) => {
        ...
        return (
          <g key={`star-${i}`}>
            <line
              x1={100 + 44 * Math.cos(a1)}
              y1={100 + 44 * Math.sin(a1)}
              x2={100 + 44 * Math.cos(a1 + Math.PI)}
              y2={100 + 44 * Math.sin(a1 + Math.PI)}
              stroke="currentColor"
              strokeWidth="0.7"
            />
            <line
              x1={100 + 54 * Math.cos(a2)}
              y1={100 + 54 * Math.sin(a2)}
              x2={100 + 54 * Math.cos(a2 + Math.PI)}
              y2={100 + 54 * Math.sin(a2 + Math.PI)}
              stroke="currentColor"
              strokeWidth="0.5"
            />
          </g>
      ...
      {/* Middle diamonds */}
      {diamonds.map((pts, i) => (
        <polygon key={`d-${i}`} points={pts} stroke="currentColor" strokeWidth="0.8" fill="none" />
      ))}

      {/* Inner lotus ring */}
      {innerPetals12.map((d, i) => (
        <path key={`ip-${i}`} d={d} stroke="currentColor" strokeWidth="0.9" fill="none" />
      ))}
...
/**
 * Ink Splatter dots matching reference watercolor artwork.
 */
export const InkSplatterOverlay: React.FC<{ color?: string }> = ({ color = '#C9974B' }) => {
  const dots: { x: string; y: string; r: number; op: number }[] = [
    { x: '62%', y: '28%', r: 1.4, op: 0.5 },
    { x: '68%', y: '31%', r: 0.9, op: 0.4 },
    { x: '71%', y: '26%', r: 1.1, op: 0.45 },
    { x: '75%', y: '33%', r: 1.6, op: 0.55 },
    { x: '78%', y: '29%', r: 0.7, op: 0.35 },
    { x: '65%', y: '37%', r: 0.8, op: 0.38 },
    { x: '60%', y: '55%', r: 1.0, op: 0.42 },
    { x: '63%', y: '60%', r: 5.5, op: 0.7 }, // Bold accent drop
    { x: '67%', y: '62%', r: 1.3, op: 0.45 },
    { x: '72%', y: '57%', r: 0.9, op: 0.38 },
    { x: '55%', y: '65%', r: 1.2, op: 0.4 },
...
/**
 * Full page background layer with two slowly-rotating corner mandalas.
 */
export const UnifiedMandalaBackground: React.FC = () => {
  return (
...
export default UnifiedMandalaBackground;
