"""Document digitizer: extracts text via Tesseract OCR with a text-fallback."""
import io
import re
...
class DocumentDigitizer:
    ...
    def parse_clinical_entities(self, text: str) -> dict:
        drugs = [
            {"form": match[0], "name": match[1], "dosage": "As directed", "frequency": "OD"}
            for match in re.findall(r"\b(Tab|Cap|Syr|Inj)\.?\s+([A-Za-z0-9-]+)", text, re.I)
        ]
        ...
        for index, medication in enumerate(drugs):
            if index < len(dosages):
                medication["dosage"] = dosages[index]
            if index < len(frequencies):
                medication["frequency"] = frequencies[index]
        return {
            "raw_text": text,
            "extracted_medications": drugs,
