...
import { Activity, ArrowUpRight, AlertTriangle, CalendarDays, CheckCircle2, ChevronRight, CircleDollarSign, Clock3, MoreHorizontal, RefreshCw, UserRound, Video } from 'lucide-react'
...
export default function DashboardPage() {
  ...
  const { data: queue, isLoading: queueLoading, mutate: refreshQueue } = useQueue()
    ...
    <div className="max-w-[1500px] mx-auto space-y-7 pb-10">
      ...
      <section className="gold-card overflow-hidden">
        <div className="p-5 md:p-6 flex justify-between items-start">
          <div>
            <p className="soft-label">Real-time kiosk intake</p>
            <h2 className="font-display text-2xl font-semibold mt-1">Live OPD Queue</h2>
          </div>
          <button
            onClick={() => void refreshQueue()}
            className="outline-button p-2 flex items-center gap-1 text-xs"
        ...
        <div className="overflow-x-auto">
          {queueLoading ? (
            <div className="px-6 py-8 text-center text-[#8A745A] text-sm">Loading queue…</div>
          ) : !queue || queue.length === 0 ? (
            <div className="px-6 py-8 text-center text-[#8A745A] text-sm">No active consultations in queue.</div>
          ) : (
            <table className="w-full text-left min-w-[620px]">
              <thead className="bg-[#F0DEC0]/55 text-[#8A745A] text-[10px] uppercase tracking-[.12em]">
                <tr>
                  <th className="px-5 py-3 font-extrabold">Patient</th>
                  <th className="px-3 py-3 font-extrabold">Status</th>
                  <th className="px-3 py-3 font-extrabold">Red Flags</th>
                  <th className="px-3 py-3 font-extrabold">Started</th>
              ...
              <tbody>
                {queue.map((item) => (
                  <tr
                    ...
                    className="border-t border-[#E8D9BC]/70 cursor-pointer hover:bg-[#F0DEC0]/30 transition-colors"
                    onClick={() => router.push(`/patient/${item.consultation_id}`)}
                  >
                    <td className="px-5 py-3.5">
                      <div className="flex items-center gap-3">
                        <div className="w-9 h-9 rounded-full bg-[#E8D9BC] flex items-center justify-center text-xs text-[#8A745A] font-extrabold">
                          {item.patient_name.split(' ').map((n: string) => n[0]).join('').slice(0, 2)}
                        </div>
                        <p className="font-bold text-sm">{item.patient_name}</p>
                      </div>
                    </td>
                    <td className="px-3 py-3">
                      <span className={`rounded-full px-2.5 py-1 text-[10px] font-extrabold ${
                        item.status === 'priority_review' ? 'bg-[#F5D5CE] text-[#A7685D]' :
                        item.status === 'awaiting_review' ? 'bg-[#F3E2C4] text-[#9A712F]' :
                    ...
                    <td className="px-3 py-3">
                      {item.red_flag_count > 0 ? (
                        <span className="flex items-center gap-1 text-[#A7685D] text-xs font-extrabold">
                          <AlertTriangle size={13} /> {item.red_flag_count}
                        </span>
                      ) : (
                        <span className="text-[#8CA383] text-xs">—</span>
                      )}
                    </td>
                    <td className="px-3 py-3 text-xs text-[#8A745A]">
                      {new Date(item.created_at).toLocaleTimeString([], { hour: '2-digit', minute: '2-digit' })}
                    </td>
                    <td className="px-5 py-3 text-right">
                      <ChevronRight size={16} className="text-[#B8863C] inline" />
                    </td>
                  </tr>
                ))}
              </tbody>
      ...
      <div className="flex items-center gap-2 text-xs text-[#8A745A]"><CheckCircle2 size={15} className="text-[#8CA383]" /> All systems operational <span className="w-1 h-1 rounded-full bg-[#C9974B]" /> Live queue active</div>
    </div>
  )
}
